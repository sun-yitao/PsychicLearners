...
new_train = train[['translated_title', 'formatted_category']]
new_valid = valid[['translated_title', 'formatted_category']]
new_train = new_train.append(new_valid)
...
with open(os.path.join(data_dir, '_train_split.txt'), 'r') as infile, \
        open(os.path.join(data_dir, big_category + '_train_invert.txt'), 'w') as outfile:
    data = infile.read()
    data = data.replace('"', '')
    outfile.write(data)
"""
with open(os.path.join(data_dir, '_valid_split.txt'), 'r') as infile, \
        open(os.path.join(data_dir, big_category + '_valid_invert.txt'), 'w') as outfile:
    data = infile.read()
    data = data.replace('"', '')
    outfile.write(data)"""
