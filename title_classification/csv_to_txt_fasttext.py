...
new_valid.to_csv(os.path.join(data_dir, '_valid_split.txt'),
                 header=None, index=None, sep=' ', mode='a')
combined.to_csv(os.path.join(data_dir, '_combined_split.txt'),
                header=None, index=None, sep=' ', mode='a')
...
with open(os.path.join(data_dir, '_combined_split.txt'), 'r') as infile, \
        open(os.path.join(data_dir, big_category + '_combined_split.txt'), 'w') as outfile:
    data = infile.read()
    data = data.replace('"', '')
    outfile.write(data)
...
os.remove(os.path.join(data_dir, '_combined_split.txt'))
