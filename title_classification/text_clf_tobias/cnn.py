...
class CNNClassfier(object):
    ...
    def build_graph(self):
        ...
        with tf.variable_scope("discriminator"):
            ...
            self.h_pool_flat = tf.layers.flatten(self.h_pool)  # batch * total_num
...
if __name__ == '__main__':
   ...
   # load data
    ...
    os.makedirs(f"{BIG_CATEGORY}/cnn", exist_ok=True)
    max_acc = 0
    ...
    for e in range(config["train_epoch"]):
        ...
        if dev_acc > max_acc:
            max_acc = dev_acc
            saver.save(
                sess, f"{BIG_CATEGORY}/cnn/{dev_acc}.ckpt", global_step=e)
